...
OTA_err ota_update(ota_info *ota_info_par)
{
    ...
    int basePathWithNameLength = snprintf(NULL, 0, "%s/%s_%d", ota_inf->path, ota_inf->basename, slot);

    char *basePathWithName = malloc(basePathWithNameLength + 1);
    snprintf(basePathWithName, basePathWithNameLength + 1,  "%s/%s_%d", ota_inf->path, ota_inf->basename, slot);
    char *sha256FilePath;
    ...
    if (ota_inf->checkSHA256) {
        ...
        int sha256FilePathLength = snprintf(NULL, 0, "%s.sha256", basePathWithName);

        sha256FilePath = malloc(sha256FilePathLength  + 1);
        snprintf(sha256FilePath, sha256FilePathLength  + 1, "%s.sha256", basePathWithName);

        http_inf.path           = sha256FilePath;
        ...
        if (sha256FilePath != NULL) free(sha256FilePath);

        // Check if dowload success
        if (err != HTTP_OK) {
          free(basePathWithName);
          goto dealloc_all;
        }
    ...
    // Dowload Firmware
    int firmwareFilePathLength = snprintf(NULL, 0, "%s.bin", basePathWithName);

    firmwareFilePath = malloc(firmwareFilePathLength  + 1);
    snprintf(firmwareFilePath, firmwareFilePathLength  + 1, "%s.bin", basePathWithName);
    ...
    if (firmwareFilePath != NULL) free(firmwareFilePath);
    if (basePathWithName != NULL) free(basePathWithName);
